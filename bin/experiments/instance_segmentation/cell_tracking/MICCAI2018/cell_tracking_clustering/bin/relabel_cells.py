...
def get_image_list_orig_2d(folder):
    index = 0
    im_list = []
    while True:
        num_str = f'{index:03}'
...
def relabel_folder(folder, output_folder_prefix):
    seg, tra = get_image_list_2d(folder)
    folder = os.path.normpath(folder)
    split = folder.split(os.sep)
    print(split)
    output_folder = os.path.join(output_folder_prefix, split[-2], split[-1], 'MERGED')
    print(output_folder)
...
def process_tif_folder(folder, output_folder_prefix):
    im_list = get_image_list_orig_2d(folder)
    folder = os.path.normpath(folder)
    split = folder.split(os.sep)
    print(split)
    output_folder = os.path.join(output_folder_prefix, split[-2], split[-1])
    print(output_folder)
...
folders = glob.glob('/run/media/chris/media1/datasets/celltrackingchallenge/trainingdataset/*2D*/*_GT/')
for folder in folders:
    if not 'PSC' in folder:
        continue
    relabel_folder(folder, 'out_t')
    #process_tif_folder(folder, 'out_t')
