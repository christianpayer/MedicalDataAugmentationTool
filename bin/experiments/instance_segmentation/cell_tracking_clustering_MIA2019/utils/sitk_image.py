...
def transform_np_output_to_input(output_image, output_spacing, channel_axis, input_image_size, input_image_spacing, input_image_origin, transform, interpolator='linear', output_pixel_type=None):
    if channel_axis is not None:
        output_images = utils.np_image.split_by_axis(output_image, axis=channel_axis)
    else:
        output_images = [output_image]
